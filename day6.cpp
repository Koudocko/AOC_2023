...
using race = std::pair<long long, long long>;

race p2_input(std::string& times, std::string& distances){
  ...
  return { std::stoll(times), std::stoll(distances) };
}

std::vector<race> p1_input(std::string& times, std::string& distances){
  std::stringstream times_stream(times), distances_stream(distances);
  long long time{}, distance{};
  std::vector<race> races;

  while (times_stream >> time && distances_stream >> distance)
